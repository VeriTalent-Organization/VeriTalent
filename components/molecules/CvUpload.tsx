...
export default function CVUploadTab() {
  ...
  const CurrentStepComponent = cvSteps[currentStep]?.component;

  const handleNext = () => {
    setCurrentStep(prev => Math.min(prev + 1, cvSteps.length - 1));
  };
  ...
  const handleFinalAction = () => {
    console.log(uploadMode === 'existing' ? 'Analyzing CVs...' : 'Processing Applications...');
  };
  ...
  return (
    <div className="bg-white p-4 sm:p-6 lg:p-8 rounded-lg shadow min-h-[600px] flex flex-col">
      ...
      <StepFooter
        onBack={handleBack}
        onNext={currentStep === cvSteps.length - 1 ? handleFinalAction : handleNext}
        canGoBack={currentStep > 0}
        isLastStep={currentStep === cvSteps.length - 1}
        nextLabel={`Next: ${cvSteps[currentStep + 1]?.label}`}
        finalActionLabel={uploadMode === 'existing' ? 'Analyze CVs' : 'Process Applications'}
      />
    </div>
  );
}