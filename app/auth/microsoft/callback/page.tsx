 ...
 * 
 * This page handles the redirect from the backend after Microsoft OAuth.
 * Expected flow:
 * 1. User clicks "Login with Microsoft SSO"
 * 2. Frontend redirects to backend: /auth/microsoft
 * 3. Backend handles OAuth with Microsoft
 * 4. Backend redirects back to: /auth/microsoft/callback?token=xxx
  ...
  useEffect(() => {
    const handleCallback = async () => {
      try {
        ...
        console.log('Microsoft OAuth Callback - URL Search Params:', {
        ...
        if (code && !token) {
          ...
          try {
            token = await authService.exchangeOAuthCode('microsoft', code);
          } catch (err) {
        ...
        if (!token) {
          ...
          setError(`No authentication token received from Microsoft. ${debugInfo}. See console for details.`);
          setIsProcessing(false);
          return;
        }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="text-center">
        ...
        <Text variant="Heading" as="h2" className="mb-2 text-xl">
          {isProcessing ? 'Completing Microsoft Sign-In...' : 'Redirecting...'}
        </Text>
