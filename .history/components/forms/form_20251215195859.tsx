  ...
  const [dropdownValues, setDropdownValues] = useState<Record<string, string>>(
    Object.fromEntries(
      fields
        ...
        .map((field) => [
          field.name,
          field.dropdown?.defaultValue || field.dropdown?.options[0] || "",
        ])
    )
  );
      ...
      <div className={`space-y-6 ${classNames}`}>
        {fields.map((field) => {
          const startIcons = field.icons?.filter(
            (icon) =>
              icon.position === "start" || icon.position === "inline-start"
          );
              ...
              render={({ field: formField }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <InputGroup>
                      {/* Render start icons */}
                      {startIcons?.map((iconConfig) =>
                        renderIconAddon(iconConfig, field.name)
                      )}
